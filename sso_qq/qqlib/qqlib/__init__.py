...
class QQ:
	...
	openapiurl='http://xui.ptlogin2.qq.com/cgi-bin/xlogin'
	...
	def loginex(self):
		...
		state = re.findall('&state=(.*?)\&scope', response.url)
		g = response.text
		print g
...
#https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=101119675&redirect_uri=http://passport.jikexueyuan.com/connect/success?t=qq&state=19325e260ec40f525abcfa2545b0c9a4&scope=get_user_info
		#self.session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36 Edge/12.10240','Content-Type':'application/x-www-form-urlencoded'})
		...
		cookies = {
            ...
            'o_cookie': self.user,
             'pgv_info': 'ssid=s2966941056',
            'pgv_pvi': '1307568128',
            'pgv_pvid': '339067894',
            'pgv_si': 's7093188608',
            'pt2gguin': 'o0909066038',
            'ptcz': 'b601db2664cb502808bf392b21db1a49a01dd2e605578025d77d80b1dae00845',
            'ptisp': 'cnc',
		...
		cookies = {'ptui_loginuin=909066038; pt2gguin=o0909066038; ptcz=b601db2664cb502808bf392b21db1a49a01dd2e605578025d77d80b1dae00845; pgv_pvid=339067894; pgv_pvi=1307568128; RK=+6VSoqtMFs; pt_clientip=983b77f8260112e4; pt_serverip=4ba00aab402c447f; pgv_si=s7093188608; ptisp=cnc; uin=o0909066038; skey=@eyeqVUYRJ'}
